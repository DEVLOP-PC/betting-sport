...
//Multiple
$parisRes = mysqli_query($dbhandle, "SELECT * FROM coupon WHERE etat_pari = 'running' AND type_pari= 'integrale'");
//$parisRes = mysqli_query($dbhandle, "SELECT * FROM coupon WHERE id='225761'");
...
while($rowCoupon = mysqli_fetch_assoc($parisRes)){
    ...
    if($lost == false){
        ...
        if($notYet == false){
            ...
            $montantJoue = $rowCoupon['amount']/$rowCoupon['multiplicateur'];
            $gainSansBonus = $coteTot * $montantJoue;
